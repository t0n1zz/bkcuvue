...
class AnggotaCuDraftImportEscete implements ToModel, WithHeadingRow, WithBatchInserts, WithChunkReading
{
    use Importable;
    ...
    {
        if (
            $row['status_cif'] != 'Batal' && $row['nama'] != 'Rek Internal'){
            try {
                if ($row['no_cu'] == '32001') {
                    $ba_cu = '007';
                ...
                $no_ba = (int)str_replace(".", "", $row['no_cif']);
            ...
            $name = strlen($nama) > 50 ? substr($nama, 0, 50) : $nama;
            ...
            $no_hp =  array_key_exists('hp', $row) ?  preg_replace('/[^(\x20-\x7F)\x0A\x0D]*/', '', preg_replace('/\s+/', '', $row['hp'])) : '';
            ...
            try {
                $anggotaCuDraft = AnggotaCuDraft::create([
                    ...
                    'lembaga' => array_key_exists('tempat_kerja', $row) ? preg_replace('/[^A-Za-z0-9]/', '', $row['tempat_kerja'])  : '',
                    'jabatan' => array_key_exists('jabatan', $row) ?  strtoupper($row['jabatan']) : '',
                    'organisasi' => array_key_exists('organisasi', $row) ? $row['organisasi'] : '',
                    'ahli_waris' => preg_replace('/[^A-Za-z0-9]/', '', $ahli_waris),
                    ...
                    'nama_ibu' => array_key_exists('nama_ibu', $row) ?  preg_replace('/[^A-Za-z0-9]/', '', $row['nama_ibu']) : '',
