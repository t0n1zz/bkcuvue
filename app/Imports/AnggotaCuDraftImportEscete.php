...
class AnggotaCuDraftImportEscete implements ToModel, WithHeadingRow, WithBatchInserts, WithChunkReading,ShouldQueue
{
    ...
    {   
        ...
        $this->cu = Cu::where('no_ba', substr($row['no_cu'],-3))->select('id','no_ba')->first();
        ...
        if(!starts_with($no_cif, '0') && !$row['alasan_hapus_cif'] && $row['status_cif']=='Aktif' && !$this->anggotaCuCuDraftBA){
            try {
