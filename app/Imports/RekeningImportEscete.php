...
class RekeningImportEscete implements ToModel, WithHeadingRow, WithBatchInserts, WithChunkReading
{
    use Importable;
    ...
    public function model(array $row)
    {
        ...
        // if ($row['no_cif'] == '4902000301' || $row['no_cif'] == '4901000765') {
        try {
            ...
            $tgl_transaksi = array_key_exists('tanggal_transaksi', $row) ? $row['tanggal_transaksi'] : Carbon::now()->subMonthNoOverflow()->endOfMonth()->toDateString();
            ...
            if ($produk_cu) {
                try {
                    AnggotaProdukCuDraft::create([
                        ...
                        'tujuan' => preg_replace('/[^\x20-\x7E\x0A\x0D]*/', '', $keterangan),
