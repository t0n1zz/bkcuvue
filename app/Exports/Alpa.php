...
class Alpa implements FromArray, WithHeadings, WithCustomStartCell, WithEvents, WithTitle
{
    ...
    public function registerEvents(): array
    {
        ...
        return [
            AfterSheet::class => function (AfterSheet $event) {
                ...
                $sheet->setCellValue('C5', "LAMA WAKTU IJIN DALAM BULAN");
    ...
    public function array(): array
    {
        ...
        $alpaS = PresensiAlpa::with('aktivis')->has('aktivis')->whereYear('created_at', '=', Carbon::parse($date)->format('Y'))->select('id_user', 'id_aktivis', 'tanggal')->get();
        $izinGroupBy = $alpaS->groupBy('id_user');
        $arrayComplete = [];
        $no = 0;
                ...
                if(isset($izinGroupBy[$aktivis->id])){
                    foreach ($izinGroupBy[$aktivis->id] as $mas) {
                        if (substr($mas->tanggal, 5, 2) == "01") {
                            $jan++;
                        } elseif (substr($mas->tanggal, 5, 2) == "02") {
                            $feb++;
                        } elseif (substr($mas->tanggal, 5, 2) == "03") {
                            $mar++;
                        } elseif (substr($mas->tanggal, 5, 2) == "04") {
                            $apr++;
                        } elseif (substr($mas->tanggal, 5, 2) == "05") {
                            $mei++;
                        } elseif (substr($mas->tanggal, 5, 2) == "06") {
                            $jun++;
                        } elseif (substr($mas->tanggal, 5, 2) == "07") {
                            $jul++;
                        } elseif (substr($mas->tanggal, 5, 2) == "08") {
                            $ags++;
                        } elseif (substr($mas->tanggal, 5, 2) == "09") {
                            $sep++;
                        } elseif (substr($mas->tanggal, 5, 2) == "10") {
                            $okt++;
                        } elseif (substr($mas->tanggal, 5, 2) == "11") {
                            $nov++;
                        } elseif (substr($mas->tanggal, 5, 2) == "12") {
                            $des++;
                        }
                    }
