...
class RekapPulangAwal implements FromArray, WithHeadings, WithCustomStartCell, WithEvents, WithTitle
{
    ...
    public function registerEvents(): array
    {
        ...
        return [
            AfterSheet::class => function (AfterSheet $event) {
                ...
                $sheet->setCellValue('C1', "WAKTU PULANG AWAL DALAM BULAN (MENIT)");
    ...
    public function array(): array
    {
        ...
        $presensi = Presensi::with('pulang_awal')->has('pulang_awal')->where('id_cu', 0)->whereYear('created_at', '=', Carbon::parse($date)->format('Y'))->get();
            ...
            if(isset($presensiGroupBy[$aktivis->id])){
                foreach ($presensiGroupBy[$aktivis->id] as $pres) {
                    if (substr($pres->tanggal, 5, 2) == "01") {
                        if ($pres->pulang_awal->jenis == 'PRIBADI') {
                            array_push($janP, $pres->pulang_awal->lama);
                        } else {
                            array_push($janUK, $pres->pulang_awal->lama);
                        }
                    } elseif (substr($pres->tanggal, 5, 2) == "02") {
                        if ($pres->pulang_awal->jenis == 'PRIBADI') {
                            array_push($febP, $pres->pulang_awal->lama);
                        } else {
                            array_push($febUK, $pres->pulang_awal->lama);
                        }
                    } elseif (substr($pres->tanggal, 5, 2) == "03") {
                        if ($pres->pulang_awal->jenis == 'PRIBADI') {
                            array_push($marP, $pres->pulang_awal->lama);
                        } else {
                            array_push($marUK, $pres->pulang_awal->lama);
                        }
                    } elseif (substr($pres->tanggal, 5, 2) == "04") {
                        if ($pres->pulang_awal->jenis == 'PRIBADI') {
                            array_push($aprP, $pres->pulang_awal->lama);
                        } else {
                            array_push($aprUK, $pres->pulang_awal->lama);
                        }
                    } elseif (substr($pres->tanggal, 5, 2) == "05") {
                        if ($pres->pulang_awal->jenis == 'PRIBADI') {
                            array_push($meiP, $pres->pulang_awal->lama);
                        } else {
                            array_push($meiUK, $pres->pulang_awal->lama);
                        }
                    } elseif (substr($pres->tanggal, 5, 2) == "06") {
                        if ($pres->pulang_awal->jenis == 'PRIBADI') {
                            array_push($junP, $pres->pulang_awal->lama);
                        } else {
                            array_push($junUK, $pres->pulang_awal->lama);
                        }
                    } elseif (substr($pres->tanggal, 5, 2) == "07") {
                        if ($pres->pulang_awal->jenis == 'PRIBADI') {
                            array_push($julP, $pres->pulang_awal->lama);
                        } else {
                            array_push($julUK, $pres->pulang_awal->lama);
                        }
                    } elseif (substr($pres->tanggal, 5, 2) == "08") {
                        if ($pres->pulang_awal->jenis == 'PRIBADI') {
                            array_push($aguP, $pres->pulang_awal->lama);
                        } else {
                            array_push($aguUK, $pres->pulang_awal->lama);
                        }
                    } elseif (substr($pres->tanggal, 5, 2) == "09") {
                        if ($pres->pulang_awal->jenis == 'PRIBADI') {
                            array_push($sepP, $pres->pulang_awal->lama);
                        } else {
                            array_push($sepUK, $pres->pulang_awal->lama);
                        }
                    } elseif (substr($pres->tanggal, 5, 2) == "10") {
                        if ($pres->pulang_awal->jenis == 'PRIBADI') {
                            array_push($oktP, $pres->pulang_awal->lama);
                        } else {
                            array_push($oktUK, $pres->pulang_awal->lama);
                        }
                    } elseif (substr($pres->tanggal, 5, 2) == "11") {
                        if ($pres->pulang_awal->jenis == 'PRIBADI') {
                            array_push($novP, $pres->pulang_awal->lama);
                        } else {
                            array_push($novUK, $pres->pulang_awal->lama);
                        }
                    } elseif (substr($pres->tanggal, 5, 2) == "12") {
                        if ($pres->pulang_awal->jenis == 'PRIBADI') {
                            array_push($desP, $pres->pulang_awal->lama);
                        } else {
                            array_push($desUK, $pres->pulang_awal->lama);
                        }
                    }
                }
            }
