...
class RekapKeluar implements FromArray, WithHeadings, WithCustomStartCell, WithEvents, WithTitle
{
    ...
    public function registerEvents(): array
    {
        ...
        return [
            AfterSheet::class => function (AfterSheet $event) {
                ...
                $sheet->setCellValue('C1', "WAKTU KELUAR KANTOR DALAM BULAN (MENIT)");
    ...
    public function array(): array
    {
        ...
        $presensi = Presensi::with('keluar')->has('keluar')->where('id_cu', 0)->whereYear('created_at', '=', Carbon::parse($date)->format('Y'))->get();
        ...
        foreach ($model as $aktivis) {
            ...
            if(isset($presensiGroupBy[$aktivis->id])){
                foreach ($presensiGroupBy[$aktivis->id] as $pres) {
                    foreach ($pres->keluar as $keluar) {
                        if (substr($pres->tanggal, 5, 2) == "01") {
                            if ($keluar->jenis == 'PRIBADI') {
                                array_push($janP, $keluar->lama);
                            } else {
                                array_push($janUK, $keluar->lama);
                            }
                        } elseif (substr($pres->tanggal, 5, 2) == "02") {
                            if ($keluar->jenis == 'PRIBADI') {
                                array_push($febP, $keluar->lama);
                            } else {
                                array_push($febUK, $keluar->lama);
                            }
                        } elseif (substr($pres->tanggal, 5, 2) == "03") {
                            if ($keluar->jenis == 'PRIBADI') {
                                array_push($marP, $keluar->lama);
                            } else {
                                array_push($marUK, $keluar->lama);
                            }
                        } elseif (substr($pres->tanggal, 5, 2) == "04") {
                            if ($keluar->jenis == 'PRIBADI') {
                                array_push($aprP, $keluar->lama);
                            } else {
                                array_push($aprUK, $keluar->lama);
                            }
                        } elseif (substr($pres->tanggal, 5, 2) == "05") {
                            if ($keluar->jenis == 'PRIBADI') {
                                array_push($meiP, $keluar->lama);
                            } else {
                                array_push($meiUK, $keluar->lama);
                            }
                        } elseif (substr($pres->tanggal, 5, 2) == "06") {
                            if ($keluar->jenis == 'PRIBADI') {
                                array_push($junP, $keluar->lama);
                            } else {
                                array_push($junUK, $keluar->lama);
                            }
                        } elseif (substr($pres->tanggal, 5, 2) == "07") {
                            if ($keluar->jenis == 'PRIBADI') {
                                array_push($julP, $keluar->lama);
                            } else {
                                array_push($julUK, $keluar->lama);
                            }
                        } elseif (substr($pres->tanggal, 5, 2) == "08") {
                            if ($keluar->jenis == 'PRIBADI') {
                                array_push($aguP, $keluar->lama);
                            } else {
                                array_push($aguUK, $keluar->lama);
                            }
                        } elseif (substr($pres->tanggal, 5, 2) == "09") {
                            if ($keluar->jenis == 'PRIBADI') {
                                array_push($sepP, $keluar->lama);
                            } else {
                                array_push($sepUK, $keluar->lama);
                            }
                        } elseif (substr($pres->tanggal, 5, 2) == "10") {
                            if ($keluar->jenis == 'PRIBADI') {
                                array_push($oktP, $keluar->lama);
                            } else {
                                array_push($oktUK, $keluar->lama);
                            }
                        } elseif (substr($pres->tanggal, 5, 2) == "11") {
                            if ($keluar->jenis == 'PRIBADI') {
                                array_push($novP, $keluar->lama);
                            } else {
                                array_push($novUK, $keluar->lama);
                            }
                        } elseif (substr($pres->tanggal, 5, 2) == "12") {
                            if ($keluar->jenis == 'PRIBADI') {
                                array_push($desP, $keluar->lama);
                            } else {
                                array_push($desUK, $keluar->lama);
                            }
                        }
                    }
                }
