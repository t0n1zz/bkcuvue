...
class StoreRekeningDraft implements ShouldQueue
{
    ...
    public function handle()
    {
        ...
        // \DB::beginTransaction(); 
        try {
        $anggotaCuCu = AnggotaCuCu::where('cu_id',$item['id_cu'])->where('no_ba',$item['no_ba'])->select('id','anggota_cu_id')->first();
        if($anggotaCuCu){
            $anggotaProdukCu = AnggotaProdukCu::where('anggota_cu_cu_id',$anggotaCuCu->id)->where('no_rek', $item['no_rek'])->where('produk_cu_id', $item['produk_cu_id'])->select('id','saldo')->first();
        }
