...
class NotificationHelper
{
	...
	public static function diklat_cu($id_cu, $kegiatan_id, $message)
	{
	...
	public static function pertemuan_cu($id_cu, $kegiatan_id, $message)
	{
		if (Auth::user()->id_cu == 0) {
			$users = User::permission(['index_laporan_cu'])->where('id_cu', $id_cu)->where('status', 1)->where('login', '>=', \Carbon\Carbon::now()->subMonth(3))->get();
			$cu_name = ' (Puskopdit BKCU Kalimantan) ';
	...
	public static function pertemuan_cu_status($kegiatan_id, $id_cus, $message)
	{
		$users = User::permission(['index_pertemuan_cu'])->whereIn('id_cu', $id_cus)->where('status', 1)->where('login', '>=', \Carbon\Carbon::now()->subMonth(3))->get();
