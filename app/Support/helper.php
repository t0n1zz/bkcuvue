...
class Helper
{
	...
	public static function image_processing($imagepath, $thumb_width, $thumb_height, $request, $kelas, $name)
	{
		...
		if ($kelas != '' && $request == "no_image") { // no image
			\Storage::disk('s3')->delete($imagepath.$kelas . '.jpg');
			...
			if ($request != 'no_image') {
				...
				//image
				if ($width > 1920) {
					\Storage::disk('s3')->put($imagepath. $fileName,  Image::make($imageData->getRealPath())->resize(
						1920,
						null,
						function ($constraint) {
					...
					\Storage::disk('s3')->put($imagepath . $fileName,  Image::make($imageData->getRealPath())->stream());
				}
