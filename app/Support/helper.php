...
class Helper
{
	...
	public static function image_processing($imagepath, $thumb_width, $thumb_height, $request, $kelas, $name, $kelas_cloud)
	{
		$path = public_path($imagepath);
		$formatedName = '';

		if ($kelas != '' && $kelas_cloud != '' && $request == "no_image") { // no image
			\Storage::disk('google_main')->delete($kelas_cloud);
			...
			if ($request != 'no_image') {
				...
				//image
				if ($width > 1920) {
					\Storage::disk('google_main')->put($formatedName . '.jpg',  Image::make($imageData->getRealPath())->resize(
						1920,
						null,
						function ($constraint) {
					...
					\Storage::disk('google_main')->put($formatedName . '.jpg',  Image::make($imageData->getRealPath())->stream());
				}
