...
class LaporanTpController extends Controller
{
	...
	{
		$table_data = LaporanTp::with('Tp.Cu')->select(
			'laporan_tp.*',
			'tp.name as tp_name',
			...
			})->addSelect([DB::raw(LaporanTpHelper::queryPerkembangan())])->whereHas('Tp', function ($query) use ($id) {
	...
	{
		$type = "laporanTpAll";
		Excel::import(new LaporanTpDraftAllImport, request()->file('file'), null, null, $type);
