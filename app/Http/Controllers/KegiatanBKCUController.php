...
class KegiatanBKCUController extends Controller
{
	...
	public function storePeserta(Request $request, $kegiatan_tipe, $id)
	{
		...
		$cekIdExsist = KegiatanPeserta::where('aktivis_id', $request->aktivis_id)->exists();
		$cekAktivisKegiatan = KegiatanPeserta::select('kegiatan_peserta.kegiatan_id')
			->join("kegiatan", "kegiatan.id", "=", "kegiatan_peserta.kegiatan_id")
			->where('kegiatan_peserta.aktivis_id', $request->aktivis_id)
			->where('kegiatan.deleted_at', '=', NULL)->exists();
		if (!$cekAktivisKegiatan == false) {
			$aktivisKegiatan = KegiatanPeserta::select('kegiatan_peserta.kegiatan_id')
				->join("kegiatan", "kegiatan.id", "=", "kegiatan_peserta.kegiatan_id")
				->where('kegiatan_peserta.aktivis_id', $request->aktivis_id)
				->where('kegiatan.deleted_at', '=', NULL)->get();
			$tanggalDiklat = Kegiatan::where('id', $id)->select('mulai', 'selesai', 'id_kode')->get();
			$diklatLainExists = Kegiatan::where('id', $aktivisKegiatan[0]->kegiatan_id)->exists();
			if (!$cekIdExsist == false || !$diklatLainExists == false) {
				foreach ($aktivisKegiatan as $dklt) {
					$tanggalDiklatLain = Kegiatan::where('id', $dklt->kegiatan_id)->select('mulai', 'selesai', 'name', 'id_kode')->get();

					if (!($tanggalDiklatLain[0]->id_kode === $tanggalDiklat[0]->id_kode)) {
						if (($tanggalDiklat[0]->mulai <= $tanggalDiklatLain[0]->mulai && $tanggalDiklat[0]->selesai <= $tanggalDiklatLain[0]->mulai) ||
							($tanggalDiklat[0]->mulai >= $tanggalDiklatLain[0]->selesai && $tanggalDiklat[0]->selesai >=  $tanggalDiklatLain[0]->selesai)
						) {
							...
							return response()
								->json([
									'saved' => false,
									'message' => 'Maaf anda anda tidak dapat mendaftar diklat ini karena diklat "' . $tanggalDiklatLain[0]->name . '" dimana anda terdaftar dilaksanakan bersamaan dengan diklat ini'
								]);
						}
					} else {
						return response()
							->json([
								'saved' => false,
								'message' => 'Maaf anda anda tidak dapat mendaftar diklat karena anda sudah terdaftar di diklat "' . $tanggalDiklatLain[0]->name . '" yang sama dengan diklat ini'
							]);
					}
				}
			} else {
