...
class KegiatanBKCUController extends Controller
{
	...
	public function storePeserta(Request $request, $kegiatan_tipe, $id)
	{
		...
		$temp_name = $request->surat_tugas->getClientOriginalName();
		Storage::disk('public')->put('files/suratTugas/' . $temp_name,  \File::get($request->surat_tugas));

		// check peserta
		if ($semuaPesertaTerdaftar < $kegiatan->peserta_max) {
			if ($asal == 'luar') {
				$kelas = KegiatanPeserta::create($request->except('surat_tugas') + ['surat_tugas' => $temp_name]);
			} else if ($asal == 'dalam') {
				if ($id_cu != 0) {
