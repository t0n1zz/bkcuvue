...
class KegiatanCUController extends Controller
{
    ...
    public function indexBaru()
    {
        ...
        $table_data = Kegiatan::with('tempat', 'sasaran', 'Regencies')->whereIn('tipe', ['diklat_cu', 'pertemuan_cu'])->where('periode', $periode)->whereIn('status', [1, 2])->orderBy('created_at', 'desc')->take(6)->get();

        $countMulai = Kegiatan::whereIn('tipe', ['diklat_cu', 'pertemuan_cu'])->where('periode', $periode)->whereIn('status', [1, 2])->where('mulai', '>', $now)->orderBy('mulai', 'asc')->count();

        $countBuka = Kegiatan::whereIn('tipe', ['diklat_cu', 'pertemuan_cu'])->where('periode', $periode)->where('status', 2)->count();

        $countJalan = Kegiatan::whereIn('tipe', ['diklat_cu', 'pertemuan_cu'])->where('status', 4)->count();
    ...
    public function indexMulai()
    {
        ...
        $table_data = Kegiatan::with('tempat', 'sasaran', 'Regencies')->whereIn('tipe', ['diklat_cu', 'pertemuan_cu'])->where('periode', $periode)->whereIn('status', [1, 2])->where('mulai', '>', $now)->orderBy('mulai', 'asc')->take(6)->get();
    ...
    public function indexBuka()
    {
        ...
        $table_data = Kegiatan::with('tempat', 'sasaran', 'Regencies')->whereIn('tipe', ['diklat_cu', 'pertemuan_cu'])->where('periode', $periode)->where('status', 2)->take(6)->get();
    ...
    {
        $table_data = Kegiatan::with('tempat', 'sasaran', 'Regencies')->whereIn('tipe', ['diklat_cu', 'pertemuan_cu'])->where('status', 4)->advancedFilter();
    ...
    public function storePeserta(Request $request, $kegiatan_tipe, $id)
    {
        ...
        if ($pesertaTerdaftar <  $kegiatan->peserta_max) {
            ...
            if ($id_cu != 0) {
                ...
                if ($dataPeserta) {
                    ...
                    if ($diff > 2) {
                        if ($kegiatan_tipe == 'diklat_cu') {
                            NotificationHelper::diklat_cu($id_cu, $id, 'menambah peserta');
                        } else {
                            NotificationHelper::pertemuan_cu($id_cu, $id, 'menambah peserta');
                        }
                    }
                } else {
                    if ($kegiatan_tipe == 'diklat_cu') {
            ...
            return response()
                ->json([
                    'saved' => false,
                    'message' => 'Maaf anda tidak bisa mendaftarkan peserta lagi, karena jumlah maksimal peserta diklat ini adalah ' . $kegiatan->peserta_max . ' orang.',
                ]);
        }
    }
    ...
    public function batalPeserta(Request $request, $kegiatan_tipe, $id)
    {
        ...
        if ($id_cu != 0) {
            if ($request->keteranganBatal != '') {
                if ($kegiatan_tipe == 'diklat_cu') {
                    NotificationHelper::diklat_cu($id_cu, $kegiatan_id, 'membatalkan pendaftaran peserta dengan alasan ' . $request->keteranganBatal);
                } else {
                    NotificationHelper::pertemuan_cu($id_cu, $kegiatan_id, 'membatalkan pendaftaran peserta dengan alasan ' . $request->keteranganBatal);
                }
            } else {
                if ($kegiatan_tipe == 'diklat_cu') {
                    NotificationHelper::diklat_cu($id_cu, $kegiatan_id, 'membatalkan pendaftaran peserta');
                } else {
                    NotificationHelper::pertemuan_cu($id_cu, $kegiatan_id, 'membatalkan pendaftaran peserta');
                }
            }
        }
