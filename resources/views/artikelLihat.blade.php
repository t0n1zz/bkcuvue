            ...
            <ol class="breadcrumb">
                ...
                @if ($artikel->kategori)
                    <li class="breadcrumb-item"><a
                            href="{{ route('artikel.kategori', $artikel->kategori->slug) }}">{{ $artikel->kategori->name }}</a>
                    </li>
                @else
            ...
            <div class="container clearfix">
                ...
                <div class="single-post nobottommargin">
                    ...
                    <div class="entry-image bottommargin">
                        ...
                        @if (!empty($artikel->gambar) && !empty($artikelGambar))
                            <img src="{{ $artikelGambar }}" alt="{{ $artikel->name }}">
                        @else
                    ...
                    <div class="postcontent nobottommargin clearfix">
                        ...
                        <div class="entry clearfix">
                            ...
                            <ul class="entry-meta clearfix">
                                ...
                                @if ($artikel->kategori)
                                    <li><a href="{{ route('artikel.kategori', $artikel->kategori->slug) }}"><i
                                                class="icon-line-grid"></i>{{ $artikel->kategori->name }}</a></li>
                                @else
                                    <li><i class="icon-line-grid"></i>{{ '-' }}</li>
                                @endif
                                @if ($artikel->penulis)
                                    <li><a href="{{ route('artikel.penulis', $artikel->penulis->slug) }}"><i
                                                class="icon-user"></i>{{ $artikel->penulis->name }}</a></li>
                                @else
                  ...
                  ============================================= -->
                        ...
                        @if ($artikel->penulis)
                            ...
                            <div class="card">
                                <div class="card-header"><strong>Ditulis oleh <a
                                            href="#">{{ $artikel->penulis->name }}</a></strong></div>
                                <div class="card-body">
                                    <div class="author-image">
                                        @if (!empty($artikel->penulis->gambar) && is_file($imagepathPenulis . $artikel->penulis->gambar . '.jpg'))
                                            <img src="{{ asset($imagepathPenulis . $artikel->penulis->gambar . '.jpg') }}"
                                                alt="" class="rounded-circle">
                                        @else
                                            <img src="{{ asset('images/no_image_man.jpg') }}" alt=""
                                        ...
                                        <div class="mpost clearfix">
                                            <div class="entry-image">
                                                @if (!empty($item->gambar) && is_file($imagepath . $item->gambar . '.jpg'))
                                                    <a href="#"><img
                                                            src="{{ asset($imagepath . $item->gambar . 'n.jpg') }}"
                                                            alt="{{ $item->name }}"></a>
                                                @else
                                                    <a href="#"><img src="{{ asset('images/image-articlen.jpg') }}"
                                                            alt="{{ $item->name }}"></a>
                                                @endif
                                            </div>
                                            <div class="entry-c">
                                                <div class="entry-title">
                                                    <h4><a
                                                            href="{{ route('artikel.lihat', $item->slug) }}">{{ $item->name }}</a>
                                                    </h4>
                                                </div>
                                                <ul class="entry-meta clearfix">
                                                    <li><i
                                                            class="icon-calendar3"></i>{{ $item->created_at->diffForHumans() }}
                                                    </li>
                                                </ul>
                                                <div class="entry-content">
                                                    {{ str_limit(preg_replace('/(<.*?>)|(&.*?;)/', '', $item->content), 100) }}
                                                </div>
                                            </div>
                                        </div>
                                    @endforeach
                            ...
                            <div class="widget widget_links clearfix">
                                ...
                                <ul>
                                    @foreach ($artikelKategoriList as $item)
                                        <li><a href="{{ route('artikel.kategori', $item->slug) }}"">
                                                <div>{{ $item->name }}</div>
                                            </a></li>
                                    @endforeach
                                </ul>
                                                ...
                                                @foreach ($artikelsBKCUNew as $item)
                                                    <div class="spost clearfix">
                                                        <div class="entry-image">
                                                            ...
                                                            @else
                                                                <a href="#"><img
                                                                        src="{{ asset('images/image-articlen.jpg') }}"
                                                                        alt="{{ $item->name }}"></a>
                                                            @endif
                                                        </div>
                                                        <div class="entry-c">
