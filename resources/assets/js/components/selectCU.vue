      ...
      <div class="card-body">
        <div class="input-group" v-if="this.currentUser.id_cu === 0">
          ...
          <select
            ...
            :disabled="modelCuStat === 'loading'"
          >
            <option disabled value="">Silahkan pilih data</option>
            <slot></slot>
            <option
              value="semua"
              v-if="this.url !== '/kegiatanCU/create/diklat_cu'"
            >
              Semua CU
            </option>
            <option value="0" v-if="isPus">
              <span v-if="currentUser.pus">{{ currentUser.pus.name }}</span>
              <span v-else>Puskopdit</span>
            </option>
            <option disabled value="">----------------</option>
            <option v-for="cu in modelCu" :value="cu.id" v-if="cu">
              {{ cu.name }}
            </option>
          </select>
          ...
          <div class="input-group-append">
            ...
            >
              <i
                class="icon-sync"
                :class="{ spinner: modelCuStat === 'loading' }"
              ></i>
            </button>
          </div>
        </div>
