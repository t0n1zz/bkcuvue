...
export const artikelSimo = {
  ...
  actions: {
    ...
    updateUtamakan( {commit, state, dispatch}, id ){
      ...
      ArtikelSimoAPI.updateUtamakan( id )
        .then( function( response ){
          if(response.data.saved){
            commit('setUpdate', response.data);
            commit('setUpdateStat', 'success');
