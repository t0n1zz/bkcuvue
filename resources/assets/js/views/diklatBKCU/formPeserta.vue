...
<script>
	...
	export default {
		...
		methods: {
			...
			checkPeserta(){
				this.$store.dispatch('diklatBKCU/checkPeserta', [this.item.id, this.formPeserta.aktivis_id]);
			},
			save(){
				this.$validator.validateAll('formPeserta').then((result) => {
					if (result) {
						if(this.mode == 'edit'){
							...
							this.$store.dispatch('diklatBKCU/storePeserta', [this.item.id, this.formPeserta]);
						}
					}else{
						this.submited = true;
					}	
