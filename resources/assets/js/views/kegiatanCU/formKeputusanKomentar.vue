<template>
  <div>
    ...
    <div v-if="!isShowHapus">
      ...
      <template
        v-if="
          itemDataKomentarStat == 'success' && itemDataKomentar.data.length > 0
        "
      >
        <div
          class="card"
    ...
    <div v-else>
      <div class="card card-body text-center">
        ...
        <div class="d-block d-md-none">
          <button
            class="btn btn-warning btn-block pb-2"
            @click.prevent="hapus()"
          >
            <i class="icon-checkmark5"></i> Ok
          </button>
          <button
            ...
            @click.prevent="isShowHapus = false"
          >
            <i class="icon-arrow-left13"></i> Batal
          </button>
        </div>
...
export default {
  ...
  methods: {
    fetchKomentar(params) {
      this.$store.dispatch("kegiatanCU/indexKeputusanKomentar", [
        params,
        this.selected.id,
      ]);
    },
      ...
      this.$validator.validateAll("formKomentar").then((result) => {
        if (result) {
          if (this.mode == "edit") {
            ...
            this.$store.dispatch("kegiatanCU/updateKeputusanKomentar", [
            ...
            this.$store.dispatch("kegiatanCU/storeKeputusanKomentar", [
              this.kegiatan_id,
              this.formKomentar,
            ]);
          }
    ...
    hapus() {
      this.$store.dispatch(
        "kegiatanCU/destroyKeputusanKomentar",
