<template>
	<div>
		<!-- message -->
		<message v-if="errors.any('formPertanyaan') && submited" :title="'Oops, terjadi kesalahan'" :errorItem="errors.items">
		</message>
      ...
      <div class="form-group">
        ...
        <!-- textarea -->
				<ckeditor type="classic" :config="ckeditorNoImageConfig" v-model="formPertanyaan.keterangan"></ckeditor>
				...
				this.$validator.validateAll('formPertanyaan').then((result) => {
					if (result) {
						if(this.mode == 'edit'){
							...
							this.$store.dispatch('pertemuanBKCU/updatePertanyaan', [this.formPertanyaan.id, this.formPertanyaan]);
						}else{
							this.formPertanyaan.id_user = this.currentUser.id;
							this.formPertanyaan.id_cu = this.currentUser.id_cu;
							this.$store.dispatch('pertemuanBKCU/storePertanyaan', [this.kegiatan_id, this.formPertanyaan]);
						}
					}else{
						this.submited = true;
					}	
