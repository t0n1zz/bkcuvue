				...
				this.$validator.validateAll('formPertanyaan').then((result) => {
					if (result) {
						if(this.mode == 'edit'){
							...
							this.$store.dispatch('kegiatanBKCU/updatePertanyaan', [this.formPertanyaan.id, this.formPertanyaan]);
						}else{
							this.formPertanyaan.id_user = this.currentUser.id;
							this.formPertanyaan.id_cu = this.currentUser.id_cu;
							this.$store.dispatch('kegiatanBKCU/storePertanyaan', [this.kegiatan_id, this.formPertanyaan]);
						}
					}else{
						this.submited = true;
					}	
