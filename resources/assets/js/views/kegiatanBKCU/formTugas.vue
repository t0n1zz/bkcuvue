<template>
  <div>
    <!-- message -->
    <message
      v-if="errors.any('formTugas') && submited"
    ...
    >
      ...
      <div
        class="form-group"
        :class="{ 'has-error': errors.has('formTugas.name') }"
      >
        <!-- title -->
        ...
        <input
          ...
          v-model="formTugas.name"
        />

        <!-- error message -->
        <small
          class="text-muted text-danger"
          v-if="errors.has('formTugas.name')"
        >
          <i class="icon-arrow-small-right"></i>
          {{ errors.first("formTugas.name") }}
        </small>
      ...
      <div
        class="form-group"
        :class="{ 'has-error': errors.has('formTugas.tipe') }"
        v-if="mode == 'create'"
      >
        <!-- title -->
        ...
        <small
          class="text-muted text-danger"
          v-if="errors.has('formTugas.tipe')"
        >
          <i class="icon-arrow-small-right"></i>
          {{ errors.first("formTugas.tipe") }}
        </small>
        <small class="text-muted" v-else>&nbsp;</small>
      </div>
      ...
      <template v-if="formTugas.tipe != ''">
        <div class="form-group" v-if="formTugas.tipe == 'google form'">
          ...
          <input
            ...
            v-model="formTugas.link"
          />
          <small class="text-muted"
            >Silahkan masukkan link ke google form</small
          >
        </div>
        ...
        <div v-else>
          <template v-if="mode == 'create'">
            ...
            <div
              class="form-group"
              :class="{ 'has-error': errors.has('formTugas.format') }"
              v-if="mode == 'create'"
            >
              <!-- title -->
              <h5 :class="{ 'text-danger': errors.has('formTugas.format') }">
                <i
                  class="icon-cross2"
                  v-if="errors.has('formTugas.format')"
                ></i>
                Pilih Format:
              </h5>
  ...
  methods: {
    ...
    save() {
      ...
      this.$validator.validateAll("formTugas").then((result) => {
        if (result) {
          if (this.mode == "edit") {
            this.$store.dispatch("kegiatanBKCU/updateTugas", [
              this.formTugas.id,
              formData,
            ]);
          } else {
            this.$store.dispatch("kegiatanBKCU/storeTugas", [
              this.kegiatan_tipe,
              this.kegiatan_id,
              formData,
            ]);
