<template>
  <div>
    <!-- message -->
    <message
      v-if="errors.any('formMateri') && submited"
    ...
    >
      ...
      <div
        class="form-group"
        :class="{ 'has-error': errors.has('formMateri.name') }"
      >
        <!-- title -->
        ...
        <small
          ...
          v-if="errors.has('formMateri.name')"
        >
          <i class="icon-arrow-small-right"></i>
          {{ errors.first("formMateri.name") }}
        </small>
      ...
      <div
        class="form-group"
        :class="{ 'has-error': errors.has('formMateri.format') }"
        v-if="mode == 'create' || mode == 'createCu'"
      >
        <!-- title -->
        ...
        <small
          class="text-muted text-danger"
          v-if="errors.has('formMateri.format')"
        >
          <i class="icon-arrow-small-right"></i>
          {{ errors.first("formMateri.format") }}
        </small>
        <small class="text-muted" v-else>&nbsp;</small>
      </div>

      <!-- upload -->
      <template v-if="mode == 'create' || mode == 'createCu'">
        <div class="form-group" v-if="formMateri.format == 'upload'">
          ...
          <small class="text-muted"
            >File yang diterima adalah PDF dan gambar/foto</small
          >
        </div>
  ...
  methods: {
    ...
    save() {
      ...
      this.$validator.validateAll("formMateri").then((result) => {
        if (result) {
          if (this.mode == "edit") {
            this.$store.dispatch("kegiatanBKCU/updateMateri", [
              this.formMateri.id,
              formData,
            ]);
          } else if (this.mode == "create") {
            this.$store.dispatch("kegiatanBKCU/storeMateri", [
              this.kegiatan_tipe,
              this.kegiatan_id,
              formData,
            ]);
          } else if (this.mode == "editCu") {
            this.$store.dispatch("kegiatanCU/updateMateri", [
              this.formMateri.id,
              formData,
            ]);
          } else if (this.mode == "createCu") {
            this.$store.dispatch("kegiatanCU/storeMateri", [
              this.kegiatan_tipe,
              this.kegiatan_id,
              formData,
            ]);
