<template>
  <div>
    <!-- message -->
    <message
      v-if="errors.any('formKomentar') && submited"
    ...
    <div class="card card-body bg-info text-white text-center">PERTANYAAN</div>
    ...
    <template v-else>
      <div v-if="!isShowHapus && !isShowJawaban && !isShowBatalJawaban">
        ...
        <template
          v-if="
            itemDataKomentarStat == 'success' &&
            itemDataKomentar.data.length > 0
          "
        >
          <div
            class="card"
            v-for="komentar in itemDataKomentar.data"
            :key="komentar.id"
            :class="{ 'border-warning': komentar.terjawab == 1 }"
          >
            <div
              class="card-header bg-warning text-white text-center"
              v-if="komentar.terjawab"
            >
            ...
            <div class="card-header bg-light header-elements-inline">
              <h6 class="card-title">
                <img
                  :src="
                    '/images/aktivis/' + komentar.user.aktivis.gambar + '.jpg'
                  "
                  width="30"
                  height="30"
                  class="rounded-circle"
                  alt="user image"
                  v-if="komentar.user.aktivis.gambar"
                />
                <img
              ...
              <div class="d-none d-sm-block">
                <div class="header-elements">
                  <span class="badge badge-secondary">
                    <small
                      v-html="$options.filters.dateTime(komentar.created_at)"
                    ></small>
                  </span>
                  <div class="list-icons ml-2">
                    <a
                      href="#"
                      class="btn btn-warning list-icons-item text-white"
                      @click.prevent="showOpen(komentar, 'jawaban')"
                      v-if="!komentar.terjawab"
                    ...
                    <a
                      href="#"
                      class="btn btn-warning list-icons-item text-white"
                      @click.prevent="showOpen(komentar, 'batalJawaban')"
                      v-if="komentar.terjawab"
                    ...
                    <a
                      ...
                      @click.prevent="showOpen(komentar, 'hapus')"
                      v-if="komentar.id_user == currentUser.id"
                      ><i class="icon-bin2"></i
                    ></a>
                  </div>
                </div>
            ...
            <div class="card-footer d-block d-sm-none text-center">
              ...
              <button
                class="btn btn-warning btn-block mt-2 mb-1"
                @click.prevent="showOpen(komentar, 'jawaban')"
                v-if="!komentar.terjawab"
              >
                <i class="icon-check"></i> Jadikan Jawaban
              </button>
              <button
                class="btn btn-warning btn-block mt-2 mb-1"
                @click.prevent="showOpen(komentar, 'batalJawaban')"
                v-if="komentar.terjawab"
              >
                <i class="icon-check"></i> Batal Jawaban
              </button>
              <button
            ...
            <div class="col-md-12 text-center">
              ...
              <div class="btn-group" v-if="itemDataKomentarStat === 'success'">
                <button
                  href="#"
                  class="btn btn-light"
                  :class="{ disabled: !itemDataKomentar.prev_page_url }"
                ...
                <button
                  ...
                  :key="index"
                  :class="{
                    'btn-primary': queryKomentar.page == n,
                    'btn-light': queryKomentar.page != n,
                  }"
                ...
                <button
                  href="#"
                  class="btn btn-light"
                  :class="{ disabled: !itemDataKomentar.next_page_url }"
                ...
                <button
                  ...
                  class="btn btn-light"
                  :class="{ disabled: !itemDataKomentar.next_page_url }"
                  @click.prevent="goToPage(itemDataKomentar.last_page)"
                >
            ...
            <div class="card-header bg-light header-elements-inline">
              <h6 class="card-title">
                <img
                  :src="
                    '/images/aktivis/' +
                    selectedKomentar.user.aktivis.gambar +
                    '.jpg'
                  "
                  width="30"
                  height="30"
                  class="rounded-circle"
                  alt="user image"
                  v-if="selectedKomentar.user.aktivis.gambar"
                />
                <img
                  src="/images/no_image_man.jpg"
                  width="30"
                ...
                />
                <check-value
                  :value="selectedKomentar.user.aktivis.name"
                  v-if="selectedKomentar.user"
                ></check-value>
                <span v-else>-</span>
                |
                <check-value
                  :value="selectedKomentar.cu.name"
                  v-if="selectedKomentar.cu"
                ></check-value>
                <span v-else>-</span>
              </h6>
              <div class="header-elements">
                <span class="badge badge-secondary">
                  <small
                    v-html="
                      $options.filters.dateTime(selectedKomentar.created_at)
                    "
                  ></small>
                </span>
              </div>
...
export default {
  ...
  methods: {
    fetchKomentar(params) {
      this.$store.dispatch("kegiatanBKCU/indexPertanyaanKomentar", [
        params,
        this.selected.id,
      ]);
    },
      ...
      this.$validator.validateAll("formKomentar").then((result) => {
        if (result) {
          if (this.mode == "edit") {
            ...
            this.$store.dispatch("kegiatanBKCU/updatePertanyaanKomentar", [
              this.formKomentar.id,
              this.formKomentar,
            ]);
          } else {
            this.formKomentar.id_user = this.currentUser.id;
            this.formKomentar.id_cu = this.currentUser.id_cu;
            this.$store.dispatch("kegiatanBKCU/storePertanyaanKomentar", [
              this.kegiatan_id,
              this.formKomentar,
            ]);
          }
    ...
    showOke() {
      if (this.isShowHapus) {
        this.$store.dispatch(
          "kegiatanBKCU/destroyPertanyaanKomentar",
          this.selectedKomentar.id
        );
      } else if (this.isShowJawaban) {
        this.$store.dispatch("kegiatanBKCU/jawabanPertanyaan", [
          this.selectedKomentar.id,
          "jawaban",
        ]);
    ...
    calculatePagination() {
      ...
      endPage = 4 + startPage;
      endPage =
        this.itemDataKomentar.last_page < endPage
          ? this.itemDataKomentar.last_page
          : endPage;
