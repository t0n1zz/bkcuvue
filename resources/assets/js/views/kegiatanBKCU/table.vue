<template>
  <div>
    ...
    <data-viewer :title="title" :columnData="columnData" :itemData="itemData" :query="query" :itemDataStat="itemDataStat"
      ...
      <!-- item desktop -->
      <template slot="item-desktop" slot-scope="props" v-if="currentUser.id_cu == 0">
      ...
      <!-- item mobile -->
      <template slot="item-mobile" slot-scope="props" v-if="currentUser.id_cu != 0">
      ...
      if (this.$route.params.periode == "semua") {
        if (this.status == "indexSemua") {
          this.$store.dispatch(this.kelas + "/index", [
            params,
            this.$route.params.tipe, this.currentUser.id_cu
          ]);
