<template>
	<div>
		...
		<data-viewer :title="title" :columnData="columnData" :itemData="itemData" :query="query"
			:itemDataStat="itemDataStat" :excelDownloadUrl="excelDownloadUrl" :isNoButtonRow="isNoButtonRow"
			:dataview="dataview" @fetch="fetch">
			...
			<template slot="button-desktop">
				...
				<template v-if="$route.params.tipe == 'diklat_bkcu' || $route.params.tipe == 'diklat_bkcu_internal'">
					<!-- tambah -->
					<router-link :to="{ name: kelas + 'Create', params: { tipe: $route.params.tipe } }"
				...
				<template
					v-else-if="$route.params.tipe == 'pertemuan_bkcu' || $route.params.tipe == 'pertemuan_bkcu_internal'">
					<!-- tambah -->
					<router-link :to="{ name: kelas + 'Create', params: { tipe: $route.params.tipe } }"
			...
			<template slot="item-desktop" slot-scope="props">
				<tr :class="{ 'bg-info': selectedItem.id === props.item.id }" class="text-nowrap"
					...
					<td v-if="!columnData[1].hide">
						<span v-html="$options.filters.statusDiklat(props.item.status)"
							@click.prevent="modalKeteranganBatalOpen(props.item.keteranganBatal)"
					...
					<td v-if="!columnData[2].hide">
						...
						<check-value :value="props.item.kode" v-else></check-value>
					</td>
					...
					<td v-if="!columnData[12].hide" class="text-warp">
						<span v-if="props.item.sasaran">
							<label v-for="(sasaran, index) in props.item.sasaran" :key="index"
				...
				<div class="col-lg-4 col-md-6">
					...
					<div class="card cursor-pointer" @click.prevent="detail(props.item.id)">
						...
						<div class="card-body">
							...
							<template
								v-else-if="props.item.tipe == 'pertemuan_bkcu' || props.item.tipe == 'pertemuan_bkcu_internal'">
							...
							<div class="row">
								<div class="col-sm-6">
									<ul class="list list-unstyled mb-1">
										...
										<li>Durasi: <span class="text-muted"><check-value
													:value="props.item.durasi"></check-value> jam</span></li>
									</ul>
								</div>
								<div class="col-sm-6">
									<ul class="list list-unstyled mb-1">
										<li>Tempat:
											...
											<span class="text-muted"
												v-else-if="props.item.tipe_tempat == 'ONLINE'">DILAKSANAKAN ONLINE</span>
											<span class="text-muted" v-else>Belum ditentukan tempat</span>
										</li>
										<li>Kabupaten/Kota: <check-value :value="props.item.regencies.name"
												v-if="props.item.regencies" class="text-muted"></check-value><span
												class="text-muted" v-else>-</span></li>
										<li>Provinsi: <check-value :value="props.item.provinces.name"
												v-if="props.item.provinces" class="text-muted"></check-value><span
												class="text-muted" v-else>-</span></li>
									</ul>
								</div>
							</div>
