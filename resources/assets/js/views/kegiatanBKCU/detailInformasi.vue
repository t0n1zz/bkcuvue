      ...
      <div class="col-lg-3 col-md-4 order-md-12">
        ...
        <div class="card">
          <div class="card-header bg-white">
            <!-- ubah  -->
            <button class="btn btn-light btn-block mb-1" @click.prevent="ubahKegiatan(item.id)"
              v-if="currentUser.can && currentUser.can['update_diklat_bkcu'] && currentUser.id_cu == 0">
              <i class="icon-pencil5"></i> Ubah
            </button>

            <!-- status -->
            <button class="btn btn-light btn-block mb-1" @click.prevent="modalOpen('statusKegiatan')"
              v-if="currentUser.can && currentUser.can['update_diklat_bkcu'] && currentUser.id_cu == 0">
              <i class="icon-calendar5"></i> Status
            </button>

            <!-- hapus -->
            <button class="btn btn-light btn-block mb-1" @click.prevent="modalOpen('hapusKegiatan')"
              v-if="currentUser.can && currentUser.can['update_diklat_bkcu'] && currentUser.id_cu == 0">
              <i class="icon-bin2"></i> Hapus
            </button>

            <!-- evaluasi -->
            <button class="btn btn-light btn-block mb-1" @click.prevent="formEvaluasi(item.id)"
              v-if="currentUser.id_cu == 0 && this.item.status == '5' && currentUser.can['update_diklat_bkcu']">
              <i class="icon-clipboard6"></i> Form Evaluasi Kegiatan
            </button>

            <!-- evaluasi -->
            <button class="btn btn-light btn-block mb-1" @click.prevent="copyLinkEvaluasi()"
              v-if="currentUser.id_cu == 0 && this.item.status == '5'">
              <span style="color: dodgerblue;"><i class="icon-clipboard2"></i> Copy Link Form
                Evaluasi Kegiatan</span>
            </button>

            <!-- daftar -->
            <template v-if="item.status != 5 && item.status != 6">
              <button class="btn bg-warning-400 btn-block mb-1" @click.prevent="tambahPeserta()"
                v-if="currentUser.can && currentUser.can['update_diklat_bkcu'] && currentUser.id_cu == 0">
        ...
        <div class="card" v-if="item.tempat">
          ...
          <div class="card-img-actions mx-1 mt-1">
            <a href="#" @click.prevent="modalImageOpen('/images/tempat/' + item.tempat.gambar + '.jpg')"
      ...
      <div class="col-lg-9 col-md-8 order-md-1">
        ...
        <div class="card">
          ...
          <div class="card-body pb-2" v-if="currentUser.id_cu == 0">
            ...
            <!-- ubah -->
            <button class="btn btn-light mb-1" @click.prevent="modalOpen('ubahListMateri')"
              :disabled="!selectedItemListMateri.id">
              <i class="icon-pencil5"></i> Ubah
            </button>
            <!-- hapus -->
            <button class="btn btn-light mb-1" @click.prevent="modalOpen('hapusListMateri')"
              :disabled="!selectedItemListMateri.id">
              <i class="icon-bin2"></i> Hapus
            </button>
          </div>
          <data-table :items="itemDataListMateri" :columnData="columnDataListMateri"
            :itemDataStat="itemDataListMateriStat">
            <template slot="item-desktop" slot-scope="props">
              <tr :class="{ 'bg-info': selectedItemListMateri.id === props.item.id }" class="text-nowrap"
              ...
              <tr v-if="props.item">
                ...
                <td v-if="props.item.pivot.asal == 'dalam'">
                  <span v-if="props.item.pekerjaan_aktif && props.item.pekerjaan_aktif.tipe == 1">
                    <check-value :front-text="'CU'" :value="props.item.pekerjaan_aktif.cu.name"
...
export default {
  ...
  methods: {
    ...
    formEvaluasi(id) {
      this.$router.push({ name: 'formEvaluasi', params: { id: id } });
    },
    copyLinkEvaluasi() {
      try {
