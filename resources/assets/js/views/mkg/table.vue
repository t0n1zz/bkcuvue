<template>
  <div>
  		<div class="card">
  			<div class="nav-tabs-responsive">
  				<ul class="nav nav-tabs nav-tabs-solid bg-light">
  					<li class="nav-item"><a href="#" class="nav-link" :class="{ 'active': tabName == 'Menunggu' }"
  							@click.prevent="changeTab('Menunggu')">
  							MENUNGGU
  						</a></li>
  					<li class="nav-item"><a href="#" class="nav-link" :class="{ 'active': tabName == 'Disetujui' }"
  							@click.prevent="changeTab('Disetujui')">
  							DISETUJUI
  						</a></li>
  				</ul>
  			</div>
    ...
    <data-viewer :title="title" :columnData="columnData" :itemData="itemData" :query="query" :itemDataStat="itemDataStat"
      :excelDownloadUrl="excelDownloadUrl"  @fetch="fetch">
      <!-- button desktop -->
      <template slot="button-desktop">
        <template>
          <!-- tambah -->
            <button @click.prevent="modalOpen('tambah')" class="btn btn-light mb-1"
          ...
          <button @click.prevent="modalOpen('edit')" class="btn btn-light mb-1"
            v-if="currentUser.can" :disabled="!selectedItem.id || selectedItem.approval_stat == 1">
      ...
      <template slot="item-desktop" slot-scope="props">
        ...
        <td v-if="!columnData[0].hide">
  					...
  					<td v-if="!columnData[2].hide">
  						<check-value :value="props.item.mkg"></check-value>
  					</td>
  					<td v-if="!columnData[3].hide && props.item.gr_d == 1">
  						<check-value :value="'I/' + props.item.gr_b"></check-value>
  					</td>
            <td v-else-if="!columnData[3].hide && props.item.gr_d == 2">
  						<check-value :value="'II/' + props.item.gr_b"></check-value>
  					</td>
            <td v-else-if="!columnData[3].hide && props.item.gr_d == 3">
  						<check-value :value="'III/' + props.item.gr_b"></check-value>
  					</td>
            <td v-else-if="!columnData[3].hide && props.item.gr_d == 4">
  						<check-value :value="'IV/' + props.item.gr_b"></check-value>
  					</td>
            <td v-if="!columnData[4].hide">
    						<span v-if="props.item.tgl_perubahan != null"
    							v-html="$options.filters.date(props.item.tgl_perubahan)"></span>
    						<span v-else>-</span>
    				</td>
            <td v-if="!columnData[5].hide && props.item.approval_stat == 1">
    						<check-value :value="'Disetujui'"></check-value>
    				</td>
            <td v-else>
    ...
    <app-modal :show="modalShow" :state="modalState" :title="modalTitle" :content="modalContent" :size="modalSize"
      ...
      <template slot="modal-body2">
  				<div>
  					<form-add :formState="method" @tutup="modalTutup" :selected="selectedItem"></form-add>
  				</div>
  			</template>
    </app-modal>
  </div>
...
export default {
  ...
  methods: {
    ...
    fetch (params) {
      ...
      this.$store.dispatch("mkg/index", [
        params, this.currentUser.id_cu, this.tabName]);
    ...
    downloadSK (id) {
      ...
      this.modalShow = true
      axios.post('/api/mkg/sk', { id: id }, {
        responseType: 'blob'
      }).then((response) => {
        FileSaver.saveAs(response.data, 'SK MKG.pdf')
    ...
    downloadSurat (id) {
      this.modalState = 'loading'
      this.modalShow = true
      axios.post('/api/mkg/suratPengajuan', { id: id }, {
        responseType: 'blob'
      }).then((response) => {
        FileSaver.saveAs(response.data, 'Surat Pengajuan Mkg.pdf')
        this.modalState = 'success';
        this.modalTitle = 'Berhasil Di Download'
      })
    },
