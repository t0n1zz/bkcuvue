		...
		<data-viewer :title="title" :columnData="columnData" :itemData="itemData" :query="query"
			...
			<template slot="button-desktop">
				...
				<router-link :to="{ name: kelas + 'Create' }" class="btn btn-light mb-1"
					v-if="currentUser.can && currentUser.can['create_surat']">
					<i class="icon-plus3"></i> Tambah
				</router-link>

				<!-- ubah-->
				<button @click.prevent="ubahData(selectedItem.id)" class="btn btn-light mb-1"
					v-if="currentUser.can && currentUser.can['update_surat']" :disabled="!selectedItem.id">
					<i class="icon-pencil5"></i> Ubah
				</button>

				<!-- hapus -->
				<button @click.prevent="modalConfirmOpen('hapus')" class="btn btn-light mb-1"
					v-if="currentUser.can && currentUser.can['destroy_surat']" :disabled="!selectedItem.id">
					<i class="icon-bin2"></i> Hapus
				</button>

				<!-- lihat -->
				<button @click.prevent="downloadMateri(selectedItem.filename)" class="btn btn-light mb-1"
					:disabled="!selectedItem.id" v-if="selectedItem.dokumen && selectedItem.dokumen.format == 'upload'">
					<i class="icon-file-eye"></i> Lihat
				</button>
				<a type="button" class="btn btn-light mb-1" :href="selectedItem.dokumen.link" target="_blank"
					v-else-if="selectedItem.dokumen && selectedItem.dokumen.format == 'link'">
					<i class="icon-file-eye"></i> Lihat
				</a>
			...
			<template slot="button-mobile">
				...
				<!-- lihat -->
				<button @click.prevent="downloadMateri(selectedItem.filename)" class="btn btn-light btn-block mb-1"
					:disabled="!selectedItem.id" v-if="selectedItem.dokumen && selectedItem.dokumen.format == 'upload'">
					<i class="icon-file-eye"></i> Lihat
				</button>
				<a type="button" class="btn btn-light btn-block mb-1" :href="selectedItem.dokumen.link" target="_blank"
					v-else-if="selectedItem.dokumen && selectedItem.dokumen.format == 'link'">
					<i class="icon-file-eye"></i> Lihat
				</a>
			...
			<template slot="item-desktop" slot-scope="props">
				<tr :class="{ 'bg-info': selectedItem.id === props.item.id }" class="text-nowrap"
					...
					<td v-if="!columnData[6].hide" v-html="$options.filters.date(props.item.tgl_terima)"></td>
