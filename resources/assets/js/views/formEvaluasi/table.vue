	...
	<div>
		...
		<data-viewer :title="title" :columnData="columnData" :itemData="itemData" :query="query"
			...
			<template slot="button-desktop">
				<!-- tambah -->
				<router-link :to="{ name: kelas + 'Create' }" @click.prevent="ubahData(selectedItem.id)"
					class="btn btn-light mb-1" v-if="currentUser.can && currentUser.can['create_diklat_bkcu']">
					<i class="icon-plus3"></i> Tambah Komponen
				</router-link>

				<!-- tambah sub-->
				<button @click.prevent="tambahSub(selectedItem.id)" class="btn btn-light mb-1"
					v-if="currentUser.can && currentUser.can['create_diklat_bkcu']" :disabled="!selectedItem.id">
					<i class="icon-plus3"></i> Tambah Sub Komponen
				</button>
				<!-- ubah-->
				<button @click.prevent="ubahData(selectedItem.id)" class="btn btn-light mb-1"
					v-if="currentUser.can && currentUser.can['create_diklat_bkcu']" :disabled="!selectedItem.id">
					<i class="icon-pencil5"></i> Ubah Komponen Evaluasi
				</button>
				<!-- hapus -->
			...
			<template slot="button-mobile">
				...
				<router-link :to="{ name: kelas + 'Create' }" class="btn btn-light btn-block mb-1"
					...
					<i class="icon-plus3"></i> Tambah Komponen Evaluasi
				</router-link>
				<!-- tambah sub-->
				<button @click.prevent="tambahSub(selectedItem.id)" class="btn btn-light mb-1"
					v-if="currentUser.can && currentUser.can['create_diklat_bkcu']" :disabled="!selectedItem.id">
