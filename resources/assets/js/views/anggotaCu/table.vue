  ...
  <div>
    ...
    <data-viewer :title="title" :columnData="columnData" :itemData="itemData" :query="query" :itemDataStat="itemDataStat" :excelDownloadUrl="excelDownloadUrl" @fetch="fetch">
      ...
      <template slot="button-mobile">
        ...
        <!-- ubah produk -->
        <button @click.prevent="ubahData(selectedItem.id,'produk')" class="btn btn-light btn-icon btn-block pb-1" v-if="currentUser.can && currentUser.can['update_anggota_cu']"
          :disabled="!selectedItem.id">
          <i class="icon-pencil5"></i> Ubah Produk
        </button>
      ...
      <template slot="item-desktop" slot-scope="props">
          ...
          <td v-if="!columnData[21].hide" v-html="$options.filters.date(props.item.tanggal_masuk)">
					</td>
