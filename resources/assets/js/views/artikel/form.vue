								...
								<div class="row">
									...
									<div class="col-md-4" v-if="currentUser.id_cu === 0">
										<div class="form-group" :class="{ 'has-error': errors.has('form.id_cu') }">
											...
											<!-- select -->
											<select class="form-control" name="id_cu" v-model="form.id_cu"
												data-width="100%" v-validate="'required'" data-vv-as="CU"
												...
												<option value="0"><span v-if="currentUser.pus">{{ currentUser.pus.name
														}}</span> <span v-else>PUSKOPCUINA</span></option>
									...
									<div class="col-md-4">
										<div class="form-group"
											:class="{ 'has-error': errors.has('form.id_artikel_penulis') }">
											...
											<div class="input-group">
												...
												<!-- select -->
												<select class="form-control" name="id_artikel_penulis"
													v-model="form.id_artikel_penulis" data-width="100%"
													v-validate="'required'" data-vv-as="Penulis"
													:disabled="modelPenulis.length === 0">
													<option disabled value="">
														<span
															v-if="form.id_cu != 0 && modelPenulis.length == 0">Silahkan
															tambah penulis baru</span>
														<span
															v-else-if="form.id_cu == '' && modelPenulis.length == 0">Silahkan
															pilih CU terlebih dahulu</span>
														<span v-else>
															<span v-if="modelPenulisStat === 'loading'">Mohon
																tunggu...</span>
															<span v-else>Silahkan pilih penulis</span>
														</span>
													</option>
													<option v-for="penulis in modelPenulis" v-if="penulis"
														:value="penulis.id">{{ penulis.name }}</option>
												</select>

												<!-- button -->
												<div class="input-group-append">
													<button type="button" class="btn btn-light"
														@click="modalOpen_Penulis" :disabled="form.id_cu === ''">
											...
											<small class="text-muted text-danger"
												v-if="errors.has('form.id_artikel_penulis')">
												<i class="icon-arrow-small-right"></i> {{
			errors.first('form.id_artikel_penulis') }}
											</small>
											<small class="text-muted" v-else>&nbsp;</small>
										</div>
									</div>
									...
									<div class="col-md-4">
										<div class="form-group"
											:class="{ 'has-error': errors.has('form.id_artikel_kategori') }">

											<!-- title -->
											<h5 :class="{ 'text-danger': errors.has('form.id_artikel_kategori') }">
												<i class="icon-cross2"
													v-if="errors.has('form.id_artikel_kategori')"></i>
											...
											<div class="input-group">
												...
												<!-- select -->
												<select class="form-control" name="id_artikel_kategori"
													v-model="form.id_artikel_kategori" data-width="100%"
													:disabled="modelKategori.length === 0" v-validate="'required'"
													data-vv-as="Kategori">
													<option disabled value="">
														<span
															v-if="form.id_cu != 0 && modelKategori.length == 0">Silahkan
													...
													<option v-for="kategori in modelKategori" v-if="kategori"
											...
											<!-- error message -->
											<small class="text-muted text-danger"
												v-if="errors.has('form.id_artikel_kategori')">
												<i class="icon-arrow-small-right"></i> {{
			errors.first('form.id_artikel_kategori') }}
											</small>
											<small class="text-muted" v-else>&nbsp;</small>
										</div>
									</div>
									...
									<!-- terbitkan -->
									<div class="col-md-4"
										v-if="currentUser.can && currentUser.can['terbitkan_' + kelas]">
										<div class="form-group" :class="{ 'has-error': errors.has('form.terbitkan') }">
											...
											<!-- select -->
											<select name="terbitkan" data-width="100%" class="form-control"
												v-model="form.terbitkan">
									...
									<!-- utamakan -->
									<div class="col-md-4"
										v-if="currentUser.can && currentUser.can['utamakan_' + kelas]">
									...
									<div class="col-md-12">
										<div class="form-group">
											...
											<!-- imageupload -->
											<app-image-upload :image_loc="'/images/artikel/'" :image_temp="form.gambar"
